...
גודל הקלף מחושב פעם אחת ב-`GeometryReader` של `CardStackView` (`Views/Main/CardStackView.swift`):
...
**Invariant — כשל טרמינלי → retry, לא ספינר-לנצח ולא dead-end שקט.** גייט הספינר תלוי ב-`showImageSpinner` **בנפרד** מ-`isLoading`, אז כל נתיב שבו התמונה הסופית לא תגיע חייב לסיים באחת משתי צורות: או `image` נוחת (הגייט `image == nil` נסגר), או המצב נדחף במפורש ל-כשל טרמינלי (`imageLoadFailed = true`) שמכבה את הספינר וחושף overlay של **retry לחיץ** במקום preview מטושטש תקוע. שתי רשויות קובעות כשל טרמינלי, משלימות זו את זו: (1) callback עם `isFinal == true` ו-`frame == nil` — בקשת ה-`.highQualityFormat` עצמה נכשלה, אז מסמנים כשל מיד (רק אם `image == nil`); (2) ה-**failsafe של ~8 שניות** ב-`armImageSpinner()` (1s debounce → spinner → 7s → אם `image == nil`: `showImageSpinner = false` + `imageLoadFailed = true`), שמכסה את מקרה ה-hang האמיתי שבו ה-callback הסופי **לעולם לא מגיע** (offline בלי proxy מקומי, או iCloud תקוע) — כמו ה-failsafe של `FullScreenMediaView`.

**דגל ה-`isFinal` של `loadImage()` — למה frame נמוך-איכות **לא** נכנס ל-`image` (התיקון לסימפטום שדווח):** מאז ש-Pass 2 מעביר `onSlowNetwork` (Fix B), ה-completion יכול לירות **יותר מפעם אחת**, וה-completion מקבל כעת `(_ frame: UIImage?, _ isFinal: Bool)`:
- `isFinal == false` — ה-fallback המקומי (`.fastFormat`) בזמן שבקשת ה-iCloud המקורית עדיין בתעופה. frame כזה נכנס ל-**`thumbnailImage`** (שדרוג preview מעל ה-thumbnail של Pass 1), **לעולם לא ל-`image`** — הגייט `image == nil` שומר על הספינר דלוק כל עוד התמונה מוצגת ברזולוציה לא-סופית. זה בדיוק הבאג שתוקן: לפני הדגל, כל frame לא-nil (כולל ה-fallback) נכנס ל-`image` וכיבה את הספינר מעל תמונה מטושטשת.
- `isFinal == true`, `frame != nil` — התמונה הסופית האמיתית: מקדמים ל-`image` (crossfade מעל ה-preview), מכבים ספינר, מאפסים `imageLoadFailed`.
- `isFinal == true`, `frame == nil` — כשל טרמינלי (ראו רשות (1) לעיל).
- `isFinal == false`, `frame == nil` — ה-fallback המקומי לא הצליח לייצר frame; מתעלמים (המקורית עדיין בתעופה, הספינר נשאר). כל delivery סופי לא-`nil` מאפס `imageLoadFailed = false`, כך ש-frame שנוחת אחרי שה-failsafe כבר סימן כשל מחליף את ה-overlay בתמונה.
...
`CardStackView` קורא `cachedScore(for: item.id)` רק כשה-ID ב-set הזה — מונע חישוב סינכרוני מתוך ה-render.  
  ...
  → loadImage completion → Task @MainActor → scheduleScore(item:image:)
        → DispatchQueue.global: score(for:image:)  ← VNFeaturePrint חוסם; חייב GCD
              ...
              → DispatchQueue.main: loadedScoreIDs.insert(id)
                    → CardStackView re-render → PhotoCardView מקבל aestheticScore != nil
...
**Interior gesture yield.** `analyze(_:maxConcurrency:yield:)`/`featurePrints` מקבלים closure אופציונלי `yield: (() async -> Void)?` שנקרא כל `featurePrintYieldInterval` (15) feature-prints שהושלמו, **לפני** הוספת עבודה חדשה — כך שמחווה (swipe) שמתחילה באמצע batch מצמצמת את הסריקה ל-`maxConcurrency` ה-in-flight בלבד במקום להריץ chunk שלם של 500 ברצף. זהו התיקון הישיר ל-freeze שדווח בהתקנה ראשונית: `prescanBatches`/`accurateBurstCount` בדקו `waitForGestureIdle` רק ב-**גבול ה-chunk**, ובהתקנה ראשונית (cache ריק) כל 500 הם חישוב Vision אמיתי — chunk בודד היה חלון רב-שנייתי חסום-GPU/ANE בלי backoff, שמרעיב את ה-compositor. ה-hook הוא closure רגיל (לא reference ל-ViewModel) כדי ש-`BurstAnalyzer` יישאר מנותק; הוא נקרא ב-parent drain loop (לא ב-child task) ולכן לא צריך `@Sendable`. מחוברים: `accurateBurstCount` (Phase 2 count) וה-prescan של `photoLibraryDidChange` מעבירים `{ await waitForGestureIdle(viewModel:) }`; נתיב ה-browse האינטראקטיבי `scanUntilFull` מעביר `nil` (המשתמש מחכה בדיוק לכרטיסי ה-burst האלה — gating יעכב את התוכן שביקש). Blur לא הושפע (CIEdges על thumb 200×200 קל בהרבה מ-feature print, ומדפדף ב-30).
...
`SmartFiltersView` קורא ל-`refreshCategoryCounts()` רק בשלושה מקרים: טעינה ראשונה ה-session (`stackViewModel.needsInitialCountRefresh` — property ב-`PhotoStackViewModel`, `FilterCategory.allCases.contains { categoryCounts[$0] == nil }` — **לא** `categoryCounts.isEmpty`, ראו הסבר למטה), אחרי שפעולת swipe/undo סימנה `hasPendingCountUpdate` והמשתמש חזר לטאב, או pull-to-refresh מפורש — לא בכל `onAppear`. `SmartFiltersView` הוא ה-caller **היחיד** של `needsInitialCountRefresh` — `resetAndLoad()` כבר לא קורא לו (ראו "באג רביעי" למטה).
...
**באג שלישי, קריטי יותר, שנתפס ב-code review:** התיקון למעלה תוקן רק ב-`SmartFiltersView`'s `.task` — אבל `resetAndLoad()` (שרץ מ-`init()` עצמו, בכל cold start!) היה עם **4 מקומות נוספים** (`if categoryCounts.isEmpty { refreshCategoryCounts() }`) עם אותו באג בדיוק, לא תוקנו. עבור משתמש ראשון-אי-פעם (או כל cache-miss — קובץ `categoryCounts.json` נמחק/פגום), `categoryCounts` ריק באמת ב-`init()`, אז ה-`isEmpty` שם היה עדיין `true` ומריץ Phase 2 מלא — vision/CIFilter כבד — **ישר ב-cold start**, בדיוק מה שכל התיקון הזה נועד למנוע. תוקן (באותו סבב) ע"י מיצוי הבדיקה ל-property משותף — `needsInitialCountRefresh`.

**באג רביעי, שהתיקון הקודם בעצמו גרם לו:** `needsInitialCountRefresh` בודק אם *כל* 6 הקטגוריות מולאו — אבל `.all`/`.screenshots`/`.screenRecordings` **לעולם לא נשמרות לדיסק**, כך שהבדיקה הזו היא `true` בתחילת **כל** cold start, אצל **כל** משתמש, לא רק בהתקנה ראשונה. לפני התיקון הקודם, `categoryCounts.isEmpty` היה במקרה `false` אצל משתמש חוזר (בזכות 3/6 הקטגוריות מהדיסק) אז ה-fallback ב-`resetAndLoad()` בפועל אף פעם לא ירה עבורו. אחרי המעבר ל-`needsInitialCountRefresh` המדויק יותר, ה-fallback הזה התחיל לירות **בכל cold start אצל כולם** — בדיוק אחרי טעינת העמוד הראשון של הכרטיסים, ממש בזמן שהמשתמש מתחיל להחליק. תוקן ע"י **הסרה מלאה** של 4 ה-fallbacks מ-`resetAndLoad()` — אין שום צורך אמיתי בהם: שום דבר חוץ מ-`SmartFiltersView` לא קורא ל-`categoryCounts`, אז dict לא-מלא פשוט נשאר לא-מלא (לא תקוע, לא שבור) עד שהמשתמש בפועל פותח את הטאב הזה.

### Pre-scan ברקע + Invalidation
`startBackgroundBlurBurstPrescan()` **כבר לא נקרא מ-`startOnboardingScan()`/cold start** (Approach B — ראו "Blurry/Burst prescan מנותק מ-onboarding" למטה). הטריגר היחיד שנשאר הוא נתיב ה-assets-החדשים ב-`photoLibraryDidChange` — שם הוא זול ואינקרמנטלי (cache-first, רק assets חדשים עולים משהו) ולעולם לא חופף לסשן ה-swipe של ההתקנה הראשונה. `.background` priority, `maxConcurrency: 3` (נמוך מה-default של 6 שמשמש נתיבים אינטראקטיביים), pagination זהה ל-`scanUntilFull`. ה-cache מתחמם עכשיו lazily: פתיחת Smart Filters → Phase 2 של `refreshCategoryCounts()`, או browse של קטגוריית Blurry/Burst → `scanUntilFull`. Cache-first הופך ריצות חוזרות לזולות כמעט לחינם.

### CPU Spike ב-Onboarding (תוקן)
`startOnboardingScan()` ו-`startBackgroundBlurBurstPrescan()` היו יורים **במקביל** מ-`OnboardingView.requestPermission()`, בדיוק בזמן שהמשתמש עדיין באנימציות של SwipeDemo/Scan — שניהם כבדים (concurrentPerform ללא cap + 6 CIFilter/Vision concurrent) וגרמו ל-~160% CPU עם jank נראה לעין. התיקון: (1) sequencing — ה-prescan נקרא רק מסוף ה-Task של `startOnboardingScan()`, לא במקביל; (2) עדיפות ה-Task הראשי ירדה מ-`.utility` ל-`.background`; (3) `DispatchQueue.concurrentPerform` (בלי cap) הוחלף ב-`TaskGroup` מוגבל (cap 4); (4) concurrency של ה-prescan ירד ל-3 (הנתיב האינטראקטיבי נשאר ב-6 default). **מוחלף חלקית:** ה-prescan הוסר מ-`startOnboardingScan()` לגמרי (Approach B, למטה) — אז הוא כבר לא רץ בנתיב ה-onboarding *ולא* ב-`scenePhase` recovery; ה-cache מתחמם lazily במקום. תיקוני ה-sequencing/QoS/TaskGroup-cap עדיין תקפים לעבודת ה-large-video של `startOnboardingScan()` ולנתיב ה-prescan של `photoLibraryDidChange` שנשאר.
...
### CPU Spike בהתקנה ראשונית — ה-Swipe הראשון בפועל (תוקן)

גם אחרי התיקון למעלה (sequencing במקום parallel), התקנה ראשונית עדיין מדדה spike אמיתי ב-CPU/GPU וdrop frames בגרירה במשך ~30 שניות מתחילת ה-swipe האמיתי — ונעלם לצמיתות אחרי זה, ולא חוזר בהפעלות הבאות. הסיבה הייתה מבנית, לא בעיית scheduling:

1. **`prescanBatches()` סורק ספרייה שלמה ללא cap** (בניגוד ל-Phase 2 של `refreshCategoryCounts()`, capped ל-100). בהתקנה ראשונית ה-cache של `BlurBurstCacheService` (verdicts + feature-prints) ריק לגמרי — כל asset דורש חישוב `CIFilter`/`VNFeaturePrintObservation` אמיתי במקום O(1) cache hit. בכל הפעלה הבאה אותו walk כמעט תמיד cache-hit ומהיר כמעט לחלוטין — בדיוק למה ה-jank הזה היה ייחודי להתקנה ראשונית.
2. **`AestheticScoringService.buildPersonaBlocking()`** (בניית הפרסונה לניקוד האסתטי) הוא `for` loop **סדרתי לגמרי, בלי concurrency** — עד 200 Favorites, `PHImageManager`(`isSynchronous: true`) + CIFilter + Vision אחד אחרי השני, ללא `TaskGroup` — בניגוד לכל סריקה כבדה אחרת בקוד. `startOnboardingScan()` היה עושה `await` לזה **לפני** ה-prescan, כך ששתי סריקות כבדות רצות ברצף אחת אחרי השנייה במקום להיפרס על פני זמן הניווט של המשתמש ב-onboarding.
3. **`.background` QoS לא מוריד עדיפות GPU/Neural-Engine**, רק עדיפות thread על ה-CPU. עבודת CIFilter/Vision מתחרה ישירות עם ה-compositor של Core Animation על אותם משאבים בדיוק בפריימים שבהם גרירה צריכה להגיע ל-120Hz.

**התיקון:**
- **`scheduleDeferredPersonaBuild()`** מחליף את הקריאות הישירות ל-`analyzeFavorites()` מ-`resetAndLoad()` ומ-`startOnboardingScan()` — `Task.detached(priority: .utility)` עם דגל `personaBuildScheduled` שמונע כפילויות, ישן 5 שניות לפני שקורא בפועל ל-`analyzeFavorites()`. `analyzeFavorites()` עצמה כבר idempotent (`persona?.isReady`/`isAnalyzing`), אז קריאה מכמה נקודות cold-start בטוחה; הדגל מתאפס אחרי כל ניסיון (גם ניסיון-ריק, למשל אם ההרשאה עדיין לא ניתנה) כדי שניסיון מוקדם מדי לא יחסום ניסיון אמיתי מאוחר יותר. `startBackgroundBlurBurstPrescan()` כבר לא מחכה לזה בכלל.
- **הקטנת גודל ה-chunk ב-`prescanBatches()` — blurry בלבד**: 300→30 (כל verdict בלתי-תלוי לגמרי, אין עלות לאיכות). **burst נשאר 500**, מיושר בכוונה עם ה-pageSize של `accurateBurstCount()`: ניסיון קודם הוריד אותו ל-100, אבל מאחר ש-`BurstAnalyzer.analyze()` חסר state חוצה-batch, chunk קטן יותר עלול לחתוך burst ארוך אמיתי על גבול עמוד — ומאחר ששני נתיבי סריקת ה-burst כותבים לאותו cache (`BlurBurstCacheService`), artifact מסריקת רקע (עדיפות נמוכה) יכול לזהם את הספירה המדויקת שהמשתמש בפועל רואה. השארת שניהם ב-500 מבטיחה ששני הנתיבים תמיד מסכימים על גבולות ה-chain. כל חמשת ערכי הכיוונון (עיכוב בניית פרסונה, מרווח polling, השהיה בין chunks, גדלי chunk) הם קבועים בעלי-שם ב-`PhotoStackViewModel.ScanTuning`, לא ליטרלים inline.
- **Gesture-priority guard**: `PhotoStackViewModel.isUserInteracting` הוא `Bool` רגיל, **לא** `@Published`, שנקבע ישירות מ-`CardStackView`'s ה-drag/pinch handlers (אותו pattern כמו `viewModel.cancelPrefetch()` הקיים כבר שם). `prescanBatches()` בודק אותו לפני כל chunk וממתין (polling, 150ms) כל עוד הוא `true`, פלוס השהיה קבועה של 100ms בין chunks בכל מקרה. חשוב: **לא** `@Published` בכוונה — `ObjectWillChange` הוא per-object לא per-property, אז flag כזה כ-`@Published` היה מפעיל re-diff מלא של ה-`ForEach` של הקלפים בכל פריים גרירה, בדיוק אותו mechanism שגרם לרגרסיה שתועדה תחת "Swipe Gesture Performance" (Round 4) ב-CLAUDE.md.

### Blurry/Burst prescan מנותק מ-onboarding (Approach B — התיקון הסופי ל-freeze של ההתקנה הראשונה)
כל המיטיגציות למעלה **ריסנו** את ה-prescan אבל השאירו אותו רץ במהלך סשן ה-swipe של ההתקנה הראשונה. משתמש דיווח שהתקנה ראשונית עדיין קופאת למספר שניות אחרי כל אחת מ-2 ההחלקות הראשונות (ונפתר לגמרי בהפעלה מחדש — החתימה של סריקה חד-פעמית, cache-ריק, ייחודית להתקנה ראשונה). שורש הבעיה: ה-gesture-priority guard בודק `waitForGestureIdle` רק ב-**גבול ה-chunk** (`prescanBatches`), וה-chunk של burst הוא 500; בהתקנה ראשונית כל 500 הם `VNFeaturePrintObservation` אמיתי, אז chunk בודד הוא חלון רב-שנייתי חסום-GPU/ANE בלי backoff פנימי, שמרעיב את Core Animation. הסריקה באמת off-main (`.background` `Task.detached`), אז זו הרעבת compositor מתחרות GPU/ANE, לא חסימת main-thread — ובדיוק לכן `.background` QoS לא מנע אותה (QoS מוריד עדיפות CPU, לא Metal/Neural-Engine). תוקן מבנית ע"י **הסרת `startBackgroundBlurBurstPrescan()` מ-`startOnboardingScan()` לגמרי** — משטח ה-swipe הראשי של Day-0 כבר לא מתחרה עם walk של Vision על כל הספרייה. חימום מוקדם של מסך ה-Smart Filters ה*משני* לא הצדיק את המחיר: ה-cache מתחמם עכשיו lazily on-demand (פתיחת Smart Filters → `refreshCategoryCounts()` Phase 2, או browse → `scanUntilFull`), פלוס אינקרמנטלית דרך ה-prescan של `photoLibraryDidChange` שנשאר. ביקור ראשון ב-Smart Filters מציג את הערכות Phase-1 המיידיות + אפקט ה-dim/spinner ("recalculating") שכבר תוכנן בדיוק למקרה הזה (מוגבל ע"י watchdog) במקום מספר מחומם-מראש — trade מקובל ובר-שחזור. עקבי עם אותו כיוון שכבר ננקט ב-"Cold-start jank" (הסרת `refreshCategoryCounts()` מ-cold start). ה-interior-yield hook על `BurstAnalyzer.analyze` (ראו "BurstAnalyzer — פירוק מקבילי") הוא רשת הביטחון המשותפת שמגנה על שני הנתיבים ש*כן* עדיין מריצים ניתוח burst (ספירת Phase 2 on-demand וה-prescan של `photoLibraryDidChange`) מאותו freeze באמצע-chunk. הזדמנות להמשך — חימום ה-cache אופורטוניסטית במעבר הראשון ל-`scenePhase → .background`, מחוץ לנתיב הקריטי — רשומה תחת "What to Build Toward" ב-CLAUDE.md.
...
> **הערה על מיקום ה-state**: `DragGesture`/`MagnificationGesture` וכל הערכים הרציפים
> שהם כותבים אליהם כל frame (`dragOffset`, `dragRotation`, `pinchScale`, `pinchOffset`)
> חיים כ-`@State` פרטי בתוך `CardStackView` (`Views/Main/CardStackView.swift`) — **לא**
> ב-`SwipeStackView`. הסיבה: `SwipeStackView` מארח גם את `SessionSavingsBarView`, badges,
> ה-FAB row (`.ultraThinMaterial`, gradients, shadows) ו-particle overlay — לו ה-state
> הרציף הזה היה חי שם, כל `.onChanged` frame (עד 120/שנייה) היה מכריח re-diff של המסך
> כולו, לא רק של הקלף. ראה CLAUDE.md → Performance Rules → "Swipe Gesture Performance"
> לפירוט המלא ולסיבת ה-CPU spike שזה תיקן. `isPinching`/`isDragging`/`isUndoAnimating`
...
**Undo (shake)** — מפוצל בין שתי views: `SwipeStackView.performUndo()` עושה את בדיקות
ה-guard וקורא ל-`viewModel.undoLastAction()`, ואז מעביר את הפעולה ל-`CardStackView`
...
SwipeStackView.performUndo()
  → guard !isDragging, !isPinching else return         ← שייק תוך כדי גרירה/pinch חי מתעלם,
                                                          לא חוטף את מצב הגסטורה של CardStackView
  ...
  → isUndoAnimating = true                             ← חוסם dragGesture וגם מנתק את pinchGesture
                                                          (ב-CardStackView: `isUndoAnimating ? nil : pinchGesture`)
  → pendingUndoRequest = UndoRequest(action: action)   ← מזוהה ב-UUID, לא רק לפי action

CardStackView.onChange(of: pendingUndoRequest) → runUndoEntry(for: action)
  → undoGeneration += 1; generation נלכד ב-let מקומי   ← undoGeneration עצמו local כאן, לא ב-SwipeStackView
  ...
  → DispatchQueue.main.asyncAfter(0.03s):              ← מאפשר לפריים הזה להיכתב
      ...
      completion: if undoGeneration == generation { isUndoAnimating = false }   ← כתיבה דרך ה-@Binding חזרה ל-SwipeStackView
...
loadImage() — שתי קריאות מקבילות:
  ...
  Pass 2: loadImage(onSlowNetwork: { }) { frame, isFinal in … }   ← Fix B: מפעיל timeout 2s → fallback מקומי → upgrade in-place
    deliveryMode = .highQualityFormat, isNetworkAccessAllowed = !isOfflineMode
    targetSize = cardTargetSize (retina-pixel dimensions)
    → ה-completion יכול לירות יותר מפעם אחת (fallback מקומי isFinal=false, ואז iCloud מקורי isFinal=true)
    → isFinal=false, frame != nil → thumbnailImage = frame (רק אם image==nil); הספינר נשאר דלוק (image==nil)
    → isFinal=false, frame == nil → מתעלמים (המקורית עדיין בתעופה)
    → isFinal=true,  frame != nil → imageLoadFailed = false; אם thumbnailImage != nil → withAnimation(.easeIn(0.18)) { image = frame } (+ asyncAfter(0.35s): thumbnailImage = nil), אחרת image = frame
    → isFinal=true,  frame == nil → כשל טרמינלי: אם image==nil → showImageSpinner=false + imageLoadFailed=true (retry overlay)
```
