...
- **`PhotoStackViewModel.startBackgroundBlurBurstPrescan()`** — **deliberately NOT triggered from onboarding/cold start (Approach B — see "Blurry/Burst prescan decoupled from onboarding" below).** Its only remaining trigger is `photoLibraryDidChange`'s newly-inserted-asset path, where it's cheap and incremental (cache-first, only new assets cost anything) and never coincides with the first-run swipe session. Walks the library at `.background` priority, `maxConcurrency: 3`. Shares a single lock (`isBlurBurstScanActive`, claimed/released via `tryAcquireBlurBurstScan()`/`releaseBlurBurstScan()`) with `refreshCategoryCounts()`'s Phase 2 blurry/burst counting — only one of the two touches `BlurBurstScanEngine`/`BurstAnalyzer` at a time; whichever loses the race skips its blur/burst work for that round rather than duplicate it.
- **`BurstAnalyzer.analyze(_:maxConcurrency:yield:)` — interior gesture yield.** `analyze`/`featurePrints` take an optional `yield: (() async -> Void)?` awaited every `featurePrintYieldInterval` (15) resolved feature prints, *before* enqueuing more work — so a swipe gesture that starts mid-batch throttles the pass to its ≤`maxConcurrency` in-flight prints instead of running an entire 500-item chunk uninterrupted. This is the direct fix for a reported first-run freeze: the burst prescan/count processes 500-item chunks (`burstPrescanPageSize`, kept at 500 for chain-boundary agreement — see below), and with an empty fresh-install cache every one of those 500 is a real `VNFeaturePrintObservation` computation; `prescanBatches`/`accurateBurstCount` only checked `waitForGestureIdle` at the *chunk boundary*, so a single chunk was a multi-second GPU/ANE-bound window with no gesture backoff, starving the compositor. The hook is a plain closure (not a ViewModel reference) so `BurstAnalyzer` stays decoupled; it's awaited in the parent drain loop (not a child task) so it needs no `@Sendable`. Wired by the two background/on-demand callers (`accurateBurstCount` at the Phase 2 count, the `photoLibraryDidChange` prescan) via `{ await waitForGestureIdle(viewModel:) }`; the interactive `scanUntilFull` browse path passes `nil` — the user is waiting on exactly those burst cards, so gating that load on gesture-idle would stall the content they asked for. Blur is unaffected (`BlurDetector` CIEdges on a 200×200 thumb is far lighter than Vision feature prints and paginates at 30) and was left without the hook.
...
- **Gesture-priority guard.** `PhotoStackViewModel.isUserInteracting` is a plain (deliberately **non-`@Published`**) `Bool`, set directly by `CardStackView`'s drag/pinch `.onChanged`/`.onEnded` handlers (mirroring the existing `viewModel.cancelPrefetch()` call already made from the same handlers). `prescanBatches()` checks it before starting each chunk and blocks (`Task.sleep` polling loop, 150ms) for as long as it's true, plus an unconditional 100ms pause between every chunk regardless. It is intentionally **not** `@Published` — `ObservableObject` invalidation is per-object, so a `@Published` flag here would fire `objectWillChange` on every gesture frame's worth of mutations and force a full re-diff of the card `ForEach`, regardless of whether any view actually reads it in `body`. This is the exact mechanism documented above under "Swipe Gesture Performance" (Round 4, `item.fileSize` in `PhotoCardView.Equatable`) that caused a real on-device regression — the fix here deliberately avoids reintroducing that class of bug.
  ...
  **Post-swipe cooldown extension.** `isUserInteracting` alone only covers the window while a finger is *physically on the card* — it clears the instant the fling's `onEnded` fires, which is precisely when the next card begins its `PhotoCardView.prepareForDisplay` decode + `.utility` prefetch. A blur/burst scan resuming at full 6-way CIFilter/Vision the moment the finger lifts contends for GPU/ANE with the very decode the user is waiting to see, so the touch-only signal left a real (if narrow) hole on the post-swipe frames. The scan-backoff gate is therefore `PhotoStackViewModel.isScanBackoffActive` (`isUserInteracting || Date().timeIntervalSince(lastSwipeAt) < ScanTuning.postSwipeCooldown`, 0.5s), **not** raw `isUserInteracting`. `lastSwipeAt` (also a plain non-`@Published` `Date`, same rationale) is stamped in `beginSwipe(_:action:)` — which `CardStackView.dragGesture.onEnded` calls synchronously the instant a fling starts, i.e. exactly when the next-card decode begins — so the scan holds off for one decode window past each swipe. `waitForGestureIdle()` polls `isScanBackoffActive`; the existing `gestureWaitTimeout` ceiling still bounds total wait, and the cooldown is inherently self-clearing (a time delta, never a stuck flag). `postSwipeCooldown` is a `TimeInterval` (compared against `Date.timeIntervalSince`), not a `Duration` like the other `ScanTuning` values (which are slept on).

**Blurry/Burst prescan decoupled from onboarding (Approach B — the definitive first-run-freeze fix):** The mitigations above (persona deferral, chunk sizing, gesture-priority guard, post-swipe cooldown) all *tamed* the onboarding-triggered prescan while leaving it running during the first-run swipe session. A user reported that a fresh install still froze the swipe stack for several seconds after each of the first couple of swipes (resolving entirely on relaunch — the signature of a one-time, empty-cache, fresh-install-only scan). Root cause confirmed: the gesture-priority guard only polls `waitForGestureIdle` at *chunk boundaries* (`prescanBatches`), and the burst chunk is 500 items; on a fresh install every one of those 500 is a real (uncached) `VNFeaturePrintObservation`, so a single chunk is a multi-second GPU/ANE-bound window with no interior backoff, starving Core Animation. The scan is genuinely off-main (`.background` `Task.detached`), so this is compositor starvation from GPU/ANE contention, not a main-thread block — which is exactly why `.background` QoS didn't prevent it (QoS deprioritizes CPU scheduling, not Metal/Neural-Engine). Fixed structurally by **removing `startBackgroundBlurBurstPrescan()` from `startOnboardingScan()` entirely** — the primary Day-0 swipe surface is never contended by a full-library Vision walk anymore. Pre-warming the *secondary* Smart Filters screen wasn't worth that cost: the blur/burst cache now warms lazily on-demand (opening Smart Filters → `refreshCategoryCounts()` Phase 2, or browsing the Blurry/Burst category → `scanUntilFull`), plus incrementally via the `photoLibraryDidChange` new-insert prescan that remains. First-ever Smart Filters visit shows the existing Phase-1 instant estimates + the dim/spinner "recalculating" affordance (already designed for exactly this first-computation case, watchdog-bounded) rather than a pre-warmed number — an accepted, recoverable trade. This is consistent with the same direction the "Cold-start swipe jank" fix already took (removing `refreshCategoryCounts()` from cold start). The interior-yield hook on `BurstAnalyzer.analyze` (see the "interior gesture yield" bullet under Blurry/Burst Scanning) is the shared safety net that additionally protects the two paths that *do* still run burst analysis (the on-demand Phase 2 count and the `photoLibraryDidChange` prescan) from the same mid-chunk freeze. A follow-up opportunity — warming the cache opportunistically on the first `scenePhase → .background` transition, off the critical path — is listed under "What to Build Toward."

**Smart Filters navigation jank (fixed):** All the fixes above hardened `startBackgroundBlurBurstPrescan()` (now new-asset-insert triggered only) against gesture contention, but `refreshCategoryCounts()`'s own Phase 2 blurry/burst path — triggered by simply opening the Smart Filters tab — was never given the same treatment, despite scanning through the identical `BlurBurstScanEngine`/`BurstAnalyzer` engines. Reported symptom: stuttery scrolling inside Smart Filters while its own Phase 2 scan ran, and severe swipe-gesture lag (>100-150% CPU) if the user navigated back to the card stack mid-scan — i.e. exactly the class of jank `isUserInteracting`-gating exists to prevent, just missing from this one call path. Two compounding issues, fixed together:
...
- **Opportunistic blur/burst cache warming, off the critical path.** Since the blur/burst prescan was decoupled from onboarding (Approach B — see "Blurry/Burst prescan decoupled from onboarding"), the cache stays cold until the user first opens Smart Filters. Recover most of the old pre-warm benefit *without* re-contending the first-run swipe session by kicking `startBackgroundBlurBurstPrescan()` (interior-yield-protected) on the first `scenePhase → .background` transition, or on an idle timer once the first swipe session settles — never during the onboarding hand-off into the stack.
